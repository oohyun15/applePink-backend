ActiveAdmin.register Review do
  ...
  filter :user, label: "#{I18n.t("activerecord.attributes.review.user_id")} 필터"
  filter :post, label: "#{I18n.t("activerecord.attributes.review.post_id")} 필터"
  filter :booking, label: "#{I18n.t("activerecord.attributes.review.booking_id")} 필터"
  ...
  show do
    attributes_table do
      row :user
      row :post
      row :booking_id do |review| review.booking end
      row :rating
      row :body
