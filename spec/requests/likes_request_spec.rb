...
describe "Like test", type: :request do
  ...
  xit "Like index test" do
    ...
    # 전체 좋아요 목록 보기
    get "/api/users/#{user.id}/likes", headers: {Authorization: @token}
    ...
    get "/api/users/#{user.id}/likes", params: {target_type: type}, headers: {Authorization: @token}
  ...
  xit "Like toggle test" do
    ...
    post "/api/users/like", params: {like: {target_id: id, target_type: type}}, headers: {Authorization: @token}
    ...
    if (target.user rescue target) == @user
      ...
      if like = @user.likes.find_by(target_type: type, target_id: target.id)
        post "/api/users/like", params: {like: {target_type: type, target_id: target.id}}, headers: {Authorization: @token}
